...
public class ChainedHashMapWithRemove {

    final int TABLE_SIZE;
    ChainedHashEntry[] table;
    public ChainedHashMapWithRemove(int size) {
        TABLE_SIZE = size;
    ...
    public void remove(int edgeSquare) {
        for (ChainedHashEntry chainedHashEntry : table) {
            ChainedHashEntry prevEntry = null;
            ChainedHashEntry entry = chainedHashEntry;

            while (entry != null) {
                if (entry.value.square() < edgeSquare) {
                    if (prevEntry == null) {
                        table[hash(entry.key)] = entry.next;
                    } else {
